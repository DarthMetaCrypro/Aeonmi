...
pub enum ASTNode {
    ...
    BinaryExpr { op: TokenKind, left: Box<ASTNode>, right: Box<ASTNode> },
    UnaryExpr { op: TokenKind, expr: Box<ASTNode> },
    Identifier(String),
    NumberLiteral(f64),
    ...
    QuantumOp { op: TokenKind, qubits: Vec<Box<ASTNode>> },
    HieroglyphicOp { symbol: String, args: Vec<Box<ASTNode>> },
...
impl ASTNode {
    ...
    pub fn new_variable_decl(name: &str, value: ASTNode) -> Self {
        Self::VariableDecl { name: name.to_string(), value: Box::new(value) }
    }
    pub fn new_assignment(name: &str, value: ASTNode) -> Self {
        Self::Assignment { name: name.to_string(), value: Box::new(value) }
    }
    pub fn new_call(callee: ASTNode, args: Vec<ASTNode>) -> Self {
        Self::Call { callee: Box::new(callee), args: args.into_iter().map(Box::new).collect() }
    }
    pub fn new_binary_expr(op: TokenKind, left: ASTNode, right: ASTNode) -> Self {
        Self::BinaryExpr { op, left: Box::new(left), right: Box::new(right) }
    }
    pub fn new_unary_expr(op: TokenKind, expr: ASTNode) -> Self {
        Self::UnaryExpr { op, expr: Box::new(expr) }
    }
    ...
    pub fn new_while(cond: ASTNode, body: ASTNode) -> Self {
        Self::While { condition: Box::new(cond), body: Box::new(body) }
    }
    pub fn new_for(
        init: Option<ASTNode>,
    ...
    pub fn new_log(expr: ASTNode) -> Self { Self::Log(Box::new(expr)) }
    pub fn new_return(expr: ASTNode) -> Self { Self::Return(Box::new(expr)) }
    pub fn new_quantum_op(op: TokenKind, qubits: Vec<ASTNode>) -> Self {
        Self::QuantumOp { op, qubits: qubits.into_iter().map(Box::new).collect() }
    }
    pub fn new_hieroglyphic_op(symbol: &str, args: Vec<ASTNode>) -> Self {
        Self::HieroglyphicOp { symbol: symbol.to_string(), args: args.into_iter().map(Box::new).collect() }
    }
}
...
>>>>>>> 9543281 (feat: TUI editor + neon shell + hardened lexer (NFC, AI blocks, comments, tests))
    ...
    fn test_assignment_and_call_nodes() {
        ...
        assert_eq!(name, "x");
        let ASTNode::Call { callee, args } = *value else { panic!("Expected Call") };
