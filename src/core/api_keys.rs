...
fn key_material() -> [u8;32] {
    ...
    let salt = b"AEONMI_API_KEY_SALT_v1";
    let mut hasher = Sha256::new(); hasher.update(user.as_bytes()); hasher.update(host.as_bytes()); hasher.update(salt);
    let out = hasher.finalize(); let mut arr=[0u8;32]; arr.copy_from_slice(&out[..32]); arr
}

fn storage_path() -> PathBuf { config_dir().unwrap_or(std::env::temp_dir()).join("aeonmi").join("keys.json") }
...
pub fn set_api_key(provider: &str, key: &str) -> Result<(), String> {
    ...
    let entry = serde_json::json!({
        ...
        "alg": "ChaCha20",
        "nonce_ct_b64": base64::encode(&stored)
    });
...
pub fn get_api_key(provider: &str) -> Option<String> {
    ...
    let bytes = base64::decode(b64).ok()?; if bytes.len()<13 { return None; }
